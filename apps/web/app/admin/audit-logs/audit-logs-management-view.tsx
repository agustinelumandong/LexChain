...
function RowActions({ row, onView, onExport }: { row: AuditRow; onView: () => void; onExport: () => void }) {
...
export function AuditLogsManagementView({ logs, total }: { logs: AuditLog[]; total: number }) {
  ...
  const targetTypes = useMemo(() => [...new Set(rows.map((row) => row.target_type).filter((value): value is string => Boolean(value)))], [rows]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      const values = [row.id, row.user_id, row.action, row.target_type, row.target_id, row.ip_address, row.user_agent, row.detailsLabel];
      const matchesSearch = !query || values.some((value) => (value ?? "").toLowerCase().includes(query));
      const matchesTarget = targetFilter === "all" || row.target_type === targetFilter;
  ...
  const metrics = [
    { label: "Total Events", value: totalDisplay, detail: "Backend total", icon: <ListAltIcon fontSize="small" />, tone: "blue" as const },
    { label: "Returned", value: metricCounts.returned, detail: "Events in current response", icon: <ScheduleIcon fontSize="small" />, tone: "yellow" as const },
    { label: "With User ID", value: metricCounts.withUser, detail: "user_id present", icon: <ManageAccountsIcon fontSize="small" />, tone: "green" as const },
    { label: "With Target", value: metricCounts.withTarget, detail: "target_type or target_id present", icon: <StorageIcon fontSize="small" />, tone: "purple" as const },
    { label: "With Details", value: metricCounts.withDetails, detail: "details object present", icon: <ArticleIcon fontSize="small" />, tone: "orange" as const },
  ];
  ...
  return (
    <div className="flex min-h-[calc(100vh-48px)] w-full flex-col gap-5">
        ...
        <article className="flex min-h-[560px] min-w-0 flex-col overflow-hidden rounded-2xl border border-[#E4EEF9] bg-white shadow-sm shadow-[#DDEAF7]/35 xl:min-h-0">
          <div className="border-b border-[#E4EEF9] p-5">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              <Dropdown value={targetFilter} onChange={setTargetFilter} options={[{ label: "Target Type", value: "all" }, ...targetTypes.map((target) => ({ label: target, value: target }))]} />
              ...
              <tbody>
                {visibleRows.map((row) => (
                  <tr key={row.id} className="h-[68px] border-b border-[#F1F5F9] transition hover:bg-[#F8FBFF]">
                    ...
                    <td className="px-5 py-3 font-semibold text-[#0C2B49]">{row.user_id ?? "—"}</td>
                    <td className="px-5 py-3 font-semibold text-[#0C2B49]">{row.action}</td>
                    <td className="px-5 py-3 font-semibold text-[#0C2B49]">{row.target_type ?? "—"}</td>
                    <td className="px-5 py-3 font-semibold text-[#0C2B49]">{row.target_id ?? "—"}</td>
                    <td className="px-5 py-3 font-semibold text-[#0C2B49]">{row.ip_address ?? "—"}</td>
                    <td className="max-w-[220px] truncate px-5 py-3 font-semibold text-[#0C2B49]">{row.user_agent ?? "—"}</td>
                    <td className="max-w-[260px] truncate px-5 py-3 font-semibold text-[#0C2B49]">{row.detailsLabel}</td>
                    <td className="px-5 py-3">
                      <RowActions
                        row={row}
                        onView={() => { setSelectedRow(row); setModalMode("view"); }}
                        onExport={() => { exportMockRows(`audit-event-${row.id}`, [row], "json"); showToast({ title: "Audit event exported", detail: row.id }); }}
                      />
                    </td>
                  </tr>
                ))}
                {visibleRows.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-5 py-10 text-center text-sm font-semibold text-[#5B6F8A]">No audit events match the current filters.</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
      ...
      <MockModal open={modalMode === "view"} onClose={() => setModalMode(null)} title={selectedRow?.id ?? "Audit event"} description="Audit event fields from the backend response." footer={<button type="button" onClick={() => setModalMode(null)} className="w-full rounded-xl bg-[#0985E7] px-5 py-3 text-sm font-black text-white">Done</button>}>
        {selectedRow ? <div className="space-y-3 text-sm font-semibold text-[#4B6382]"><p><strong className="text-[#071B33]">User ID:</strong> {selectedRow.user_id ?? "—"}</p><p><strong className="text-[#071B33]">Action:</strong> {selectedRow.action}</p><p><strong className="text-[#071B33]">Target:</strong> {selectedRow.targetLabel}</p><p><strong className="text-[#071B33]">IP Address:</strong> {selectedRow.ip_address ?? "—"}</p><p><strong className="text-[#071B33]">User Agent:</strong> {selectedRow.user_agent ?? "—"}</p><p><strong className="text-[#071B33]">Details:</strong> {selectedRow.detailsLabel}</p></div> : null}
      </MockModal>
    </div>
  );
}
