...
import { ParticipantAccessTable } from '../../../components/participant-access-table';
import { inviteDocumentParty, listDocumentParties, revokeDocumentParty } from '../../../lib/portal-access-api';
import { canManageParticipants } from '../../../lib/participant-access';
...
export default function DocumentParticipantsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const inviteMutation = useMutation({ mutationFn: (invitation: { email: string; role: string }) => inviteDocumentParty(id, invitation), onSuccess: refreshParties });
  ...
  return (
    <div className="flex w-full min-w-0 flex-col gap-5 overflow-x-hidden">
      ...
      {profileQuery.isError || !isIssuer ? <CapabilityUnavailable message={profileQuery.isError ? 'Your account details could not be verified, so participant access is not available.' : 'Only the Document Issuer can manage participants for this document.'} /> : partiesQuery.isError ? <CapabilityUnavailable message="The document access endpoint rejected this request. No participant access changes were made." /> : <><InviteParticipantForm onInvite={(invitation) => inviteMutation.mutateAsync(invitation)} /><ParticipantAccessTable parties={partiesQuery.data?.parties ?? []} onRevoke={(party) => revokeMutation.mutateAsync(party)} /></>}
    </div>
  );
}
